    ...
    async def register(self, referral_link: str):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                analysis = self.error_analyzer.analyze_error(str(e), error_context)
                print(self.error_analyzer.format_error_report(analysis))
            ...
            try:
                if action_type == "fill":
                    ...
                    if clicked:
                        ...
                        if better_selector:
                            print(f"   🎯 Gemini предложил новый селектор: '{better_selector}'")
                            try:
                                element = await page.wait_for_selector(better_selector, timeout=5000)
                                if element and await element.is_visible():
                                    print("   ✅ Элемент найден по новому селектору!")
                                    await self.human_like_element_click(page, element)
                                    print(f"   ✅ Клик выполнен успешно!")
                                    clicked = True
                                    success_count += 1
                            except Exception as e:
                                print(f"   ⚠️ Новый селектор тоже не сработал: {str(e)[:50]}...")
                    ...
                    if not clicked:
                        ...
                        if required:
                            ...
                            # Анализируем проблему через ErrorAnalyzer
                            analysis = self.error_analyzer.analyze_error(
                                f"Element not found with selector: {selector}", 
                                error_context
                            )
                            
                            print(f"   🧠 Анализ ошибки: {analysis['summary']}")
                            
                            # Получаем рекомендации от ErrorAnalyzer
                            recovery_success = False
                            if analysis['analysis']:
                                for suggestion in analysis['analysis']:
                ...
                analysis = self.error_analyzer.analyze_error(str(e), error_context)
                print(f"   ❌ Ошибка при выполнении действия:")
                print(f"      {str(e)}")
                
                # Определяем, стоит ли повторить
                error_type = analysis['analysis'][0]['type'] if analysis['analysis'] else 'unknown_error'
                if self.error_analyzer.should_retry(error_type) and required:
                    retry_delay = self.error_analyzer.get_retry_delay(error_type, 1)
